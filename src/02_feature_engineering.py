...
def calculate_rolling_features(df):
  ...
  def get_rolling(group):
    ...
    group['roll_pts_scored'] = (
        group['pts_scored'].shift(1).rolling(4, min_periods=1).mean()
    )
    group['roll_pts_allowed'] = (
        group['pts_allowed'].shift(1).rolling(4, min_periods=1).mean()
    )
    return group
  ...
  home_stats = team_df[team_df['is_home'] == 1][
      ['game_id', 'roll_pts_scored', 'roll_pts_allowed']
  ]
...
def calculate_advanced_rolling_stats(seasons):
  ...
  team_stats['total_yards'] = team_stats['passing_yards'] + team_stats.get(
      'rushing_yards', 0
  )
  team_stats['total_plays'] = (
      team_stats['attempts']
      + team_stats['sacks_suffered']
      + team_stats.get('carries', 0)
  )
  team_stats['net_yds_per_play'] = (
      team_stats['total_yards'] / team_stats['total_plays']
  )
...
if __name__ == '__main__':
  ...
  print(
      f'\n✅ Saved {len(final_features_df)} rows of training data to'
      f' {output_path}'
  )